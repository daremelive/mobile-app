...
import { View, Text, TextInput, ScrollView, TouchableOpacity, Image, Modal, Pressable, Alert, KeyboardAvoidingView, Platform, Keyboard } from 'react-native';
...
export default function MessageDetailScreen() {
  ...
  const fetchRecipient = useCallback(async () => {
    ...
    try {
      ...
      const response = await authenticatedFetch(`${baseUrl}users/${recipientId}/`, {
        headers: {
  ...
  const handleSend = async () => {
    ...
    try {
      if (isNewConversation && recipientId) {
        ...
        if (
          !messageRequest
          || messageRequest.recipientId !== numericRecipientId
        ...
        const conversationsResponse = await authenticatedFetch(`${baseUrl}messaging/conversations/`, {
          headers: {
            'Content-Type': 'application/json',
          },
        });
