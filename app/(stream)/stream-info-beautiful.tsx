...
export default function StreamInfoScreen() {
  ...
  const handleInviteByUsername = async () => {
    ...
    try {
      // For now, we'll send invitation by username to the backend
      // The backend will handle finding the user and sending notifications
      const result = await inviteUsersToStream({ 
        streamId: streamId, 
        userIds: [], // We'll modify backend to accept usernames too
        username: inviteUsername.trim()
      }).unwrap();

      Alert.alert(
        '🎉 Invitation Sent!', 
        `Successfully invited @${inviteUsername} to join your live stream. They will receive a notification in their DareMe account and email.`
      );
      ...
      <View className="flex-1 bg-black">
        ...
        <SafeAreaView className="flex-1">
          ...
          <View className="absolute top-12 left-4 z-10">
            <View className="flex-row items-center bg-black/50 rounded-full px-3 py-2">
          ...
          <TouchableOpacity 
            onPress={handleClose}
            className="absolute top-12 right-4 w-10 h-10 bg-black/50 rounded-full items-center justify-center z-10"
          >
